...
Discord의 `exempt_channels` 제한 때문에 제외해야 할 채널이 50개를 넘으면 발송 전 차단 구성이 불가능합니다. 이 경우 `/멘션차단`에 여러 채널 ID를 한 번에 넣어 차단 대상 채널 수를 늘려야 합니다.

제외 채널이 50개를 넘어서 정확한 채널 제한을 유지할 수 없으면, 봇은 차단하지 않은 채널이 막히지 않도록 기존 `역할 멘션 차단 (bot-managed)` AutoMod 규칙을 삭제합니다. 이 상태에서는 차단 대상 채널을 충분히 추가한 뒤 다시 `/멘션차단`을 실행해야 발송 전 차단이 켜집니다.
...
`/멘션차단초기화`는 현재 서버의 저장된 차단 채널과 봇이 만든 AutoMod 규칙을 모두 삭제합니다.
